...
planes = planes %>% select(year, tailnum) %>% 
...
data %>% group_by(name) %>% 
  ...
  geom_col(aes(x = average_depdelay, y = reorder(name, average_depdelay)),
           fill = "steelblue")+
  labs(y = "Nama Maskapai", x = "rata-rata keterlambatan (menit)",
       title = "10 maskapai Rata-rata dengan keterlambatan terlama",
       caption = my_caption)


# B---------
sblm2000 = data %>% filter(year_manufactur < 2000) %>% 
  select(distance) %>% pull()
sblm2000

sdh2000 = data %>% filter(year_manufactur >= 2000) %>% 
  select(distance) %>% pull()
sdh2000
...
glimpse(data)
data = data %>% 
  mutate(musim = case_when(month %in% c(10:12, 1)~"winter",
