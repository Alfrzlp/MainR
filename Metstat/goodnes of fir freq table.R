...
freq.table = function(data, k, i=NA ,pisah.int = F){
  if(!is.na(i)) breaks = seq(min(data), max(data), by=i)[-1]
  else breaks = seq(min(data), max(data), length=k)[-k]
  ...
  if(pisah.int){
    df %>% 
      ...
      mutate_at(c("bawah", "atas"), perbaiki) %>% return()
  }else return(df)
} 
#======================================================================
...
freq.table(df, i=1.05, pisah.int = T) %>% 
  gof.test(mean(df), sd(df)) %>% summarise_all(sum)
...
gof.test = function(df, mean, sd){
  df %>% 
    mutate(p = pnorm((atas+0.5-mean)/sd)-pnorm((bawah-0.5-mean)/sd),
           p = round(p, 5),
           ei = sum(Freq)*p) %>% 
