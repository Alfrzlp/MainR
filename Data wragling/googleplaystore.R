...
a = data.clean %>% group_by(Category) %>% 
  summarise(total_install = sum(Installs)) %>% 
  arrange(total_install) %>% 
  ggplot() +
  geom_col(aes(x=total_install, 
               y=reorder(str_to_title(Category), total_install), 
               fill=reorder(Category, total_install)))+
  scale_fill_viridis_d(direction = -1)+
  theme(legend.position = "none") +
  labs(title = "Most Popular category (by Total Installs)", 
       x = "Total Installs", y="Category")

b = data.clean %>% group_by(Category) %>% 
  count(Type) %>% filter(Type == "Paid") %>% arrange(n) %>% 
  ggplot() + geom_col(aes(x=n, 
                          y=reorder(str_to_title(Category), n),
                          fill=reorder(Category, n)))+
  geom_text(aes(x=n+3, y=reorder(str_to_title(Category), n),
                label = n)) +
  scale_fill_viridis_d(direction = -1) +
  theme(legend.position = "none") +
  labs(title = "Kategori paling banyak Aplikasi Berbayar",
