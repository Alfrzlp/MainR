...
ggplot(databaru, aes(x=year)) + geom_line(aes(y=gdpPercap, color=country, group=country))
ggplot(databaru, aes(x=year)) + geom_line(aes(y=lifeExp, color=country, group=country)) 
...
gapminder %>% arrange(desc(gdpPercap)) %>% filter(year == 2007) #urut dgp
...
library(dplyr)
airquality %>% drop_na() %>% group_by(Month) %>% 
  summarise_all(mean)
...
data = t(data.frame(x,y, jumlah=x+y)) %>% data.frame() %>% 
...
dat %>% mutate_all(~str_remove_all(.x, "\\s")) %>% #Hapus spasi
  mutate_at(2:11, ~str_replace_all(.x, ",", ".")) %>% 
  type_convert() %>%  as.data.frame()
