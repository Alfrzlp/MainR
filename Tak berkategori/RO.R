...
data.frame(
  permintaan = c(800000, 600000, 400000),
  p = c(0.2, 0.5, 0.3)
) %>%
  mutate(
    m1 = hbeli[1] + boperasional[1] * permintaan,
...
mms <- function(lambda, miu, s, nn = NULL) {
  ...
  if (!is.null(nn)) {
    for (n in 1:nn) {
      if (n > s) {
        pn <- (lambda / miu)^n * rho / (factorial(s) * s^(n - s))
      } else {
        pn <- (lambda / miu)^n * po / factorial(n)
      }
