...
desi %>% 
  ...
  select(postid = rowid, tags, 
         time = taken_at_timestamp,
         n_like = edge_media_preview_like_count,
         comment = comments_data
         ) %>% 
  mutate(tags = map_chr(tags, toString),
         time = as.POSIXct(time, origin = "1970-01-01")) %>%  #ekstrak coment
  unnest_wider(comment) %>% 
  unnest() -> desi2


desi2 %>% group_by(username) %>% 
  count() %>% ggplot2::ggplot() +
  geom_col(aes(x=reorder(username, n), y=n)) + coord_flip()

desi2 %>% select(text) %>% 
  mutate(text = gsub("@\\S*", "", text)) %>% 
  rtweet::plain_tweets() %>%
  tidytext::unnest_tokens(input = text, output = token) %>% 
  group_by(token) %>% anti_join(stopwords, by=c("token" = "stopword")) %>% 
  count(sort = T) %>% wordcloud2()
