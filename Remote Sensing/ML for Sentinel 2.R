...
setwd("D:/_Datasets/Remote sensing")
...
tc <- trainControl(
  method = "repeatedcv", # repeated cross-validation of the training data
  number = 10, # number of folds
  repeats = 5, # number of repeats
  allowParallel = TRUE, # allow use of multiple cores if specified in training
  verboseIter = TRUE
) # view the training iterations
...
nnet.grid <- expand.grid(
  size = seq(from = 2, to = 10, by = 2), # number of neurons units in the hidden layer
...
svm.grid <- expand.grid(
  sigma = seq(from = 0.01, to = 0.10, by = 0.02), # controls for non-linearity in the hyperplane
  C = seq(from = 2, to = 10, by = 2)
) # controls the influence of each support vector

## Begin training the models. It took my laptop 8 minutes to train all three algorithms
# Train the neural network model
nnet_model <- caret::train(
  x = trn[, (5:ncol(trn) - 1)], y = as.factor(as.integer(as.factor(trn$class))),
...
eva.sp <- SpatialPointsDataFrame(
  coords = cbind(eva$x, eva$y), data = eva,
