...
ggplot(market, aes(x=age, y=balance)) + geom_point()
qplot(x=age, y=balance, geom="point", data=market)

ggplot(market, aes(x=age, y=balance, color=marital)) + geom_point()

ggplot(market, aes(x=age, y=balance, color=marital)) +
  geom_point() + facet_wrap(~marital)

ggplot(market, aes(balance)) + geom_histogram() + xlab("Balance") +
  ylab("Frequency") + ggtitle("Histogram Balance")

ggplot(market, aes(balance)) + geom_histogram(fill="blue", color="red") 

ggplot(market, aes(age)) + geom_freqpoly(color="red")
...
#Some EU Countries
negara.eu = c("Portugal", "Spain", "France", "Switzerland", "Germany",
...
ggplot(negara.eu.maps, aes(x=long, y=lat)) +
  ...
  scale_fill_viridis_d() + theme_void() + theme(legend.position = "none")

indonesiamaps = map_data("world", region = "Indonesia")

region.data = indonesiamaps %>% 
  group_by(subregion) %>% summarise(long=mean(long), lat=mean(lat))
