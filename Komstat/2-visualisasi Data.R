...
plot(x, y,
  pch = 19, cex = 0.7, xlab = "waktu(detik)",
...
# cEX = Ukuran legend
legend(0, 1,
  legend = c("sin", "cos"), lty = 1:2, cex = 1,
...
plot(mtcars$wt, mtcars$mpg,
  main = "Milage vs Car weight", xlab = "weight", ylab = "Miles / gallon",
  pch = 19
)
...
grafik <- qplot(data$year[1:36], data$gdpPercap[1:36],
  main = "GDP Percapita",
...
bar <- ggplot(data = data, aes(x = data$pop[1:12], y = data$gdpPercap[1:12])) +
