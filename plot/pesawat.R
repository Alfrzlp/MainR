...
rute = read.delim("(D:/Datasets/apapun/routes.dat.txt)", sep = ",",
                  header = F)
bandara = read.delim("(D:/Datasets/apapun/airports.dat.txt)", sep = ",",
                     header = F)
...
colnames(bandara) <- c("id", "nama", "kota", "negara", "IATA", "ICAO", "lat", "long",
                       "altitude", "bedadariUTC", "DST")
colnames(rute) <- c("kode", "ID", "asal_bandara", "ID_asal_bandara", "tujuan_bandara", "ID_tujuan_bandara",
...
ggplot(indonesia) + 
  geom_map(map=indonesia, aes(x=long, y=lat, map_id=id),
...
rute_id = rute %>% 
  ...
  rename(nama_asal = nama, kota_asal = kota,
         lat0 = lat, long0 = long) %>% 
  left_join(bandara, by = c('tujuan_bandara'='IATA')) %>% 
  drop_na(lat, long) %>% 
  rename(nama_tujuan = nama, kota_tujuan = kota,
         lat1 = lat, long1 = long) %>% 
...
ggplot(indo)+
  ...
  geom_segment(data = rute_id, aes(x = long0, y = lat0,
