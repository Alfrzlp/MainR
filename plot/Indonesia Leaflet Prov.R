...
df <- readxl::read_xlsx('D:/tpt rls.xlsx',
  ...
  mutate(rls = str_replace_all(rls, ',', '.'),
...
ggplot(id, aes(x=long, y=lat)) + 
  ...
  labs(title = 'Rata-rata Lama Sekolah Penduduk Indonesia Tahun 2020',
...
pal2 = colorBin("YlOrRd", domain = indo$rls,
...
leaflet(indo) %>% 
  ...
  addLegend(pal = pal, values = indo$rls,
            title = "Rata-rata Lama Sekolah",
            position = "topright") %>% 
  addCircleMarkers(lng = 106.828303, lat = -6.223996,
                   popup = paste0(
...
ggplot() +
  ...
  xlim(95, 141) + ylim(-11, 8) +
  geom_polygon(data = indonesia, aes(x=long, y=lat,
                                     fill=maret_2020, group=group), col = 'black')+
  theme_bw()+
  scale_fill_viridis_c(direction = -1, option = 'inferno')+
  labs(title = 'Presentase Penduduk Miskin Indonesia',
       subtitle = 'Maret 2020',
...
ggplot(df, aes(x=tpt, y=september_2020)) + 
  ...
  labs(y = 'Presentase Penduduk Miskin',
