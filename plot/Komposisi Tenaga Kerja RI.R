...
df = read.table(textConnection(str)) %>% 
  ...
  type_convert() %>% 
  `colnames<-`(c('Tidak/belum pernah sekolah', 'Tidak/belum tamat SD ', 'SD', 'SLTP', 'SLTA Umum/SMU',
               'SLTA Kejuruan/SMK', 'Akademi/Diploma', 'Universitas'))

df$status <- c('Berusaha Sendiri',
...
df %>% 
  ...
  labs(fill = 'Pendidikan Tertinggi yang di tamatkan', x = 'Jumlah Penduduk', y = "Status Pekerjaan",
