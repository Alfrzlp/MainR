perempuan <- c(180, 120,	180, 360,	240, 120, 180,	120, 240, 170, 150,	120,	180,	180,	150,
               200,	150,	180,	150,	180, 120,	60,	120,	180,	180, 90,	240,	180,	115,	120)	

laki <- c(90,120,	30,	90,	200, 90,	45,	30,	120,	75, 150,	120,	60,	240,	300, 240,	60,	120,
...
library(ggplot2)
library(dplyr)
pria = df %>% group_by(Lama_Belajar, Jenis_Kelamin) %>% count() %>%
...
ggplot(pria, aes(x="", y=n, fill=Lama_Belajar)) +
  ...
  coord_polar("y", start=0) + ggtitle("Lama Belajar Mahasiswa")

pie(pria$n, col = rainbow(nrow(pria), start = 0.1, end = 0.8),
    clockwise = T)
#===========================================================================
#scater plot
ggplot(iris) +
...
ggplot(penduduk, aes(y=reorder(kelompok_umur, 1:20))) +
  ...
  scale_x_continuous(breaks = seq(-1e+07, 1e+07, 5e+06),
                     labels = c("10 juta", "5 juta", "0", "5 juta", "10 juta")) +
  labs(title = "Piramida Penduduk Indonesia Tahun 2010",
...
data = data %>% mutate(posisi = if_else(mpg_z<0, "bawah", "atas")) %>% 
  arrange(-mpg_z)
data
...
ggplot(data) +
  ...
  geom_segment(aes(x=0, y=reorder(mobil, 32:1),
                   xend=mpg_z, yend=reorder(mobil, 32:1), col=posisi))+
  geom_text(aes(x=mpg_z+0.3, y=reorder(mobil, 32:1),
                label= round(mpg_z, 3), size=6)) +
  theme(legend.position = "none") + labs(y="Jenis Mobil")
...
gapminder %>% filter(year==2007) %>% 
  ...
  summarise(lifeExp = mean(lifeExp), gdpPercap = mean(gdpPercap),
            pop = mean(pop), continent) %>% ggplot() + 
  geom_point(aes(x=gdpPercap, y=lifeExp, col=continent, size=pop))


#===========================================================================
#set warna
df = data.frame(pdb = c(17475, 16298, 12855, 11347),
                Negara = c("Jepang \n tahun 1970",
                           "Singapura \n tahun 1991",
                           "Hongkong \n tahun 1982",
                           "Korea \n tahun 2000"))
...
ggplot(df) +
  ...
  scale_fill_viridis_d(direction = -1) + theme_test()+
...
animate(myplot, duration = 8, fps = 20,
...
animate(myplot2,duration = 8, fps = 20,
        renderer = gifski_renderer() )
...
library(coronavirus)
covid_indo <- coronavirus %>% filter(country == "Indonesia") %>% 
...
animate(grafik_covid, duration = 19, fps = 30, width=800,
        renderer = gifski_renderer())
...
pddk <- ggplot(data, aes(y=reorder(UMUR, 1:416))) +
  ...
  labs(title = "Tahun: {closest_state}", y = "Kelompok Umur", x="Jumlah Penduduk",
       fill = "Jenis Kelamin")+
  scale_x_continuous(breaks = c(-2e+05, 0e+00, 2e+05),
...
animate(pddk, fps = 30, duration = 10,
        renderer = gifski_renderer())
