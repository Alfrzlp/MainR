...
Jenis kelamin KRT (1: laki-laki, 0: perempuan)	87,97	91,54	89,77	90,68	88,87	89,84	87,51	90,24	87,44	86,45	87,54	89,34	87,21	89,03	86,16	84,88	84,93	87,45	86,26	80,9
...
dat <- dm::read_pattern(
  ...
  setNames(
    c('kat', 'nama', 'status',
      str_glue('desil{rep(1:10, each = 2)}_{rep(0:1, 10)}')
     )
  ) %>% 
...
sk_levels <- str_glue('desil{rep(1:10)}')
...
full_data_final %>% 
  walk2(
    ...
    ~ {
      ...
      ggsave(
        plot = p,
        filename = str_glue("E:/Visualisasi/desil/{my_names[.y]}.png"),
        width = 10,
        height = 5,
        units = "in", dpi = 300,
        scale = 0.85, bg = 'white'
