...
ubah = function(str) return(eval(parse(text=str)))
...
randvar = function(expr, xmin, xmax, ymin, ymax, new_exp="1",thd="x", ds="kontinu"){
  ...
  Nama = c("E(X)","E(Y)","E(X^2)","E(Y^2)", "E(XY)", n, "Var(X)", "Var(Y)", "Cov(X,Y)",
...
ui <- fluidPage( 
  ...
  tabsetPanel(
    ...
    tabPanel("Operasi",
             sidebarLayout(
               sidebarPanel(h4("Masukkan Fungsi"),
                            
                            textInput("fungsi", "", value = "exp(-x-y)"),
                            textInput("x", strong("interval x"), value = "0,oo"),
                            textInput("y", strong("Interval y"), value = "0,oo"),
                            fluidRow(column(width = 8, selectInput('metode', "", width = "120px",
                                                 choices = list("Integral" = 1,
                                                                "Differensial" = 2,
                                                                "Sum" = 3,
                                                                "Limit" = 4),
                                                 selected = 1)),
                                    column(width = 4, selectInput('thd', "", width = "120px",
                                                 choices = list("x" = 1, 
                                                                "y" = 2,
                                                                "x y" = 3,
                                                                "y x" = 4),
    ...
    tabPanel("Transformasi",
             sidebarLayout(
               sidebarPanel(h3("Fungsi"),tags$hr(),
...
server = function(input, output){
  ...
  observeEvent(input$tombol, {
    ...
    tryCatch(
      ...
      }, error = function(a){ yint = 0 }
    )
    
    tryCatch(
      { xint = as.vector(strsplit(isolate(input$x), ",")[[1]])
      }, error = function(a){ xint = 0}
    )
  ...
  observeEvent(input$tombol2, {
    ...
    if(isolate(input$select2) == 1 | isolate(input$select2) == 3) thd = "x"
    else thd = "y"
    
    if(isolate(input$select2) == 4 | isolate(input$select2) == 3) d = "diskret"
    else d = "kontinu"
    ...
    output$table = renderTable({
      ...
      tryCatch(
        ...
        }, error = function(a){ table = data.frame(Error = "Terdapat Kesalahan Input")}
      )
