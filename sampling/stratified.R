...
alokasi.stratified(df, alokasi = "sebanding", 0.95) %>% 
  mutate(v = (1-nh/Nh)*sh^2*(Nh/3000)^2/nh) %>% summarise(sum(v))
...
sampel.stratified(df, alokasi = "optimum", 0.95, total_biaya = 20000000,
           biaya_tetap = 4000000, minim_biaya = F)

#=========================================================================================
strata = list(laki = c(7,10,3,6,14,5),
              perempuan = c(14,18,21,10,16))
strata2 = list(laki = c(1,0,1,1,0,1),
              perempuan = c(0,1,0,0,1))
Nh = c(19, 17)


olah = function(lis, Nh, N, ps = "wor"){
  if(ps == "wor") data$fh = 1-(data$nh/data$Nh)
  else fh = 0
      
  data = data %>% mutate("v(ybarh)" = (1-fh)*varh/nh,
