...
asal_negara <- sampel %>%
  ...
  mutate(
    proporsi = jumlah_penumpang / n, Total = N * proporsi,
...
# estimasi proporsi
data1 %>%
  transmute(kelas = cut(data1, c(0, 3, 5, Inf), c(1, 2, 3))) %>%
  group_by(kelas) %>%
  count() %>%
  rename(jumlah = n) %>%
  mutate(
    p = jumlah / nrow(data1), varians_rata = f * p * (1 - p) / (n - 1),
...
# estimasi jumlah
data1 %>%
  transmute(kelas = cut(data1, c(0, 100, Inf), c(1, 0))) %>%
  group_by(kelas) %>%
  count() %>%
  rename(jumlah = n) %>%
  mutate(
...
f <- ((N - n) / N)
data1 %>%
  transmute(kelas = cut(data1, c(0, 30, Inf), c(1, 0))) %>%
  group_by(kelas) %>%
  count() %>%
  rename(jumlah = n) %>%
  mutate(
