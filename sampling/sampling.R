systematic_sampling <- function(data, n, metode="l", kosong=0){
  ...
  for(i in start:ngs){
    pos = i
    for(j in 1:n){
      if((pos>N)&(metode=="l")) gugus_sample[j,i] <- kosong
      else if((pos<=N)&(metode=="l")) gugus_sample[j,i] <- as.numeric(data[(pos)])
  ...
  tryCatch({gugus_sample = gugus_sample[,-exe]},
...
data = c(0, 0.9, 0,0, 0.3, 0.1, 0.5, 3, 2.8, 2.7, 2.8, 2.6, 2.3, 3.5, 2.4,
