...
pp_baru = c()
for(i in df$b7r25){
  if(i < 250000) pp_baru = append(pp_baru, 1)
  else pp_baru = append(pp_baru, 0)
}
sum(pp_baru)
#------------------------------------------------------------------------------------
#recode
atap_baru = c()
for(i in df$b6r2){
  if(i == 'Genteng'| i=='Seng' | i=='Beton') atap_baru = append(atap_baru, 1)
  else atap_baru = append(atap_baru, 0)
}
atap_baru
#------------------------------------------------------------------------------
library(dplyr)
...
data %>% arrange(desc(gdpPercap)) %>% filter(year == 2007)

databaru
ggplot(databaru, aes(year, gdpPercap)) + geom_line() + geom_point() + labs(x="Year",y="Gdp Percap", title = "Congo, Dem, Rep")
