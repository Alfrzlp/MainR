...
estCS <- function(all_cluster, N, cara = 2, Mbar = NA){
  ...
  vybarn = (N-n)*sb2/(N*n)
  df1 <- data.frame(nilai = c(ybarn, vybarn, sqrt(vybarn), sqrt(vybarn)*100/ybarn),
                    row.names = c("ybar n", "v(ybar n)", "se(ybar n)", "rse(ybar n)"))
  
  ycapn = N*Mbar*ybarn
  vycapn = (N*Mbar)^2*vybarn 
  df2 <- data.frame(nilai = c(ycapn, vycapn, sqrt(vycapn), sqrt(vycapn)*100/ycapn),
...
estPPS_CS(get_allCluster(str), N = 40, X = 4000,
