...
information_gain <- function(y, x, func = minimize_fun) {
  ...
  if (s1 == 0 | s2 == 0) {
    ...
    return(func(y) - s1 / s * func(y[x]) - s2 / s * func(y[!x]))
  }
}
...
decision_tree <- function(formula, data, max_depth = 3, min_n = 20, min_ig = 1e-3, minimize_fun = "gini_impurity") {
  if (minimize_fun == "gini_impurity") {
    minimize_fun <- gini_impurity
  } else if (minimize_fun == "variance") {
...
data.tree::FromListSimple(.change_name(hasil))
str(hasil)
...
preds <- predict1(iris[, -5], hasil)
cm <- table(actual = iris$species, pred = preds)
cm
sum(diag(cm)) / sum(cm)
...
# package -----------------------------------------------------------------
tree::tree(species ~ ., data = iris, minsize = 40, mincut = 20, mindev = 1e-7)
rpart::rpart(species ~ ., data = iris, maxdepth = 3, minbucket = 20, minsplit = 40, cp = 1e-9)



# minsize = The smallest allowed node size: a weighted quantity. The default is 10
# dimana 2 * mincut = minsize
tree::tree(
  Kyphosis ~ .,
  data = kyphosis,
